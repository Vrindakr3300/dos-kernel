...
| **A4** — terminology pass (PDP/advisory over kernel/reference-monitor) | **CARRIED** into the framing above; strategy-prose edits live in `dos-strategy` | "PDP with no PEP" is now the load-bearing internal description (this section + the future apply-gate plan, docs/119). The README/tagline rewording is a `dos-strategy` task per the CLAUDE.md one-way-arrow rule, not a kernel-`docs/` edit. |
