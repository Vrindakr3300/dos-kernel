...
| "Install it" (to *use*) | **`dos-kernel` is NOT on PyPI yet (as of 2026-06-10)** — install from this clone: `pip install -e .` (runtime, PyYAML-only) or `pip install -e ".[mcp]"` (adds the MCP server). Never `pip install dos` — that bare name is an unrelated squatter package. The full matrix (uv, pipx, wrappers, WSL) is [docs/INSTALL.md](docs/INSTALL.md). |
